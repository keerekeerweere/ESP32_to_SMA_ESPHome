...
class ESP32SmaBluetooth;

class ESP32_SMANetArduino : public ESP32Loggable {
    public:
         ESP32_SMANetArduino() : ESP32Loggable("ESP32_SMANetArduino") {
        ...
        void writeSMANET2Short(unsigned char *btbuffer, unsigned short v);
        ...
        void writeSMANET2PlusPacket();
        void writeSMANET2PlusPacket(unsigned char *btbuffer, unsigned char longwords, unsigned char ctrl, unsigned short ctrl2, unsigned short dstSUSyID, unsigned long dstSerial);
        ...
        void writePacketHeader(unsigned char *btbuffer, const unsigned int control, unsigned char *destaddress);
        void writePacketHeader(unsigned char *btbuffer, const unsigned int control, unsigned char *myaddress, unsigned char *destaddress);
...
//        void writePacketHeader(unsigned char *btbuffer, unsigned char cmd1, unsigned char cmd2, unsigned char *destaddress);
        ...
        bool start(); //passthrough
        bool isConnected(); //passthrough
        unsigned char* getsmaBTInverterAddressArray();

    protected:
        ESP32SmaBluetooth smaBluetooth;


        //correctly ordered inverter address for sma communication (human read in reverse)
...
//        unsigned char sixff[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
        unsigned char address_unknown[6]= {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

        unsigned char Level1SrcAdd[6];
        unsigned char Level1DestAdd[6];

        prog_uchar PROGMEM SMANET2header[4] = {0xFF, 0x03, 0x60, 0x65};
        size_t sizeof_SMANET2header = sizeof(SMANET2header);

        prog_uchar PROGMEM InverterCodeArray[6] = {0x5c, 0xaf, 0xf0, 0x1d, 0x50, 0x00}; // Fake address on the SMA NETWORK
        size_t sizeof_InverterCodeArray = sizeof(InverterCodeArray);

        prog_uchar PROGMEM fourzeros[4] = {0, 0, 0, 0};
    ...
    friend class ESP32SmaBluetooth;
    friend class ESP32_SMA_Inverter;
};
